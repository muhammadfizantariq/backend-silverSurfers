...
/**
 * Checks if a box area has actual visual content.
 * @param {string | Buffer} imagePathOrBuffer The image to check.
 * @param {object} rect The rectangular area to analyze.
...
/**
 * Extracts box data specifically from the interactive-color-audit.
...
function extractBoxData(lighthouseReport, auditId) {
    ...
    for (const item of audit.details.items) {
        // Reads coordinates from the 'node.boundingRect' property
...
export async function processInteractiveColorAudit(jsonReportPath) {
    const AUDIT_ID = 'interactive-color-audit';
    const outputImagePath = './highlighted-interactive-color.png';
